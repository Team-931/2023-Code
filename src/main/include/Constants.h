...
/**
 * The Constants namespace provides a good place for robot-wide constant
 * variables like port numbers, drive measurements, and subsystem
 * configurations. This can also be used to hold numerical or boolean constants.
 * This should not be used to hold any dynamic code.
 *
 * It is generally a good idea to place constants into subsystem-specific
 * namespaces within this header, which can then be used where they are needed.
 */
namespace Constants
{
    ...
    namespace DriveTrain
    {
        ...
        // X-axis is oriented in the forward direction
        constexpr double driveLength = 24.25, driveWidth = 24.125;
        constexpr double halfLen = driveLength / 2,
                         halfWid = driveWidth / 2;
        constexpr double offsetXs[]{halfLen, halfLen, -halfLen, -halfLen};
        constexpr double offsetYs[]{halfWid, -halfWid, -halfWid, halfWid};
        const double rotationRescale = std::sqrt(std::pow(halfLen, 2) + std::pow(halfWid, 2));

        // Motor hardware and gearing constants
        constexpr int encoderResolution = 2048;
        constexpr double
            turnGearing = 72.0 / 14 * 24 / 12,
            ticksPerRotation = turnGearing * encoderResolution,
        ...
        // Wheel encoder value offset (0 is forward)
        constexpr int absSubtraction[]{2220, 2148, 2695, 1023};
    ...
    namespace Arm
    {
        ...
        constexpr double CtlP = 0.1, CtlF = 0.5 * 1024 / maxVel;
    }
}